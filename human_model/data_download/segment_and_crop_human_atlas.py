    ...
    for i in range(1, np.max(labeled_nuclei)):
        ...
        if np.sum(current_nuc) > min_nuc_size:
            ...
            if c1 < 0 or c3 < 0 or c2 > image_shape[0] or c4 > image_shape[1]:
                ...
                Image.fromarray(nuclei_crop).save(outfolder + "//" + outimagename + "_blue.tif")
                Image.fromarray(antibody_crop).save(outfolder + "//" + outimagename + "_green.tif")
                Image.fromarray(microtubule_crop).save(outfolder + "//" + outimagename + "_red.tif")
...
if __name__ == "__main__":
    ...
    filepath = "../human_protein_atlas/"                            # Path of the downloaded HPA images
    outpath = "../human_protein_atlas_single_cell/"                 # Path to save the crops to
    outfile = "../human_protein_atlas_single_cell_centers.txt"      # Path to save file of cell center coordinates to
    ...
    for directory in os.listdir(filepath):
        for image in os.listdir(filepath + directory):
            ...
            find_centers_and_crop (imagepath, directory, image, outpath, outfile)